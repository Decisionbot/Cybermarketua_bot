...
from default_commands import DeviceStatesGroup, CooldownStatesGroup, ParametersStatesGroup
...
# # @dp.message_handler(text_contains="✅ Це все, зберігти зображення", state=DeviceStatesGroup.photo)
...
# @dp.message_handler(content_types=types.ContentType.ANY, state=DeviceStatesGroup.photo)
...
# # @dp.message_handler(text_contains="📩 Опублікувати", state=DeviceStatesGroup.name)
...
# сохранения + ожидание ввода пользователем / новый вопрос + смена state
# @dp.message_handler(content_types=types.ContentType.CONTACT, state=DeviceStatesGroup.name)
...
def register_handlers_device(dp: Dispatcher):
    dp.register_message_handler(cmd_create, text_contains="💻 Девайс", state=ParametersStatesGroup.parameter)
    dp.register_message_handler(load_desc, state=DeviceStatesGroup.description)
    dp.register_message_handler(load_price, state=DeviceStatesGroup.price)
    dp.register_message_handler(photo_confirm, text_contains="✅ Зберігти зображення",
                                state=DeviceStatesGroup.photo)
    dp.register_message_handler(load_photo, content_types=types.ContentType.ANY, state=DeviceStatesGroup.photo)
    dp.register_message_handler(public_confirm, text_contains="📩 Опублікувати", state=DeviceStatesGroup.name)
    dp.register_message_handler(load_name, content_types=types.ContentType.CONTACT, state=DeviceStatesGroup.name)
